Feature: QA Job Page
  ...
  @smoke
  Scenario: Qa List job postings and verify that the job application page is opened2
    Then verify that "home page" is visible successfully
    When select "Company" main menu on homepage
    And select "Careers" submenu on homepage
    Then verify that "careers" is visible successfully
  ...
  @smoke
  Scenario: Qa List job postings and verify that the job application page is opened3
    Then verify that "home page" is visible successfully
    When select "Company" main menu on homepage
    And select "Careers" submenu on homepage
    Then verify that "careers" is visible successfully
